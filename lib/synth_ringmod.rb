...
module SynthRingmod
  ...
  def generate(duration: nil, freq: nil, params: nil)
    ...
    carrier_freq = ParamOverride.fetch(o, :carrier_freq) { freq || rand(100.0..1200.0).round(2) }

    mod_ratio = ParamOverride.fetch(o, :mod_ratio) do
      [rand(0.21..0.79), rand(1.13..2.87), rand(3.01..7.21)].sample.round(4)
    end
    mod_freq = ParamOverride.fetch(o, :mod_freq_start) { (carrier_freq * mod_ratio).round(3) }

    carrier_wave = ParamOverride.fetch(o, :carrier_wave) { CARRIER_WAVES.sample }
    amplitude = ParamOverride.fetch(o, :amplitude) { rand(0.4..0.7).round(2) }
    ring_amount = ParamOverride.fetch(o, :ring_amount) { rand(0.5..1.0).round(3) }

    sweep_octaves = ParamOverride.fetch(o, :sweep_octaves) { rand(-1.0..1.0).round(2) }
    mod_end = ParamOverride.fetch(o, :mod_freq_end) { (mod_freq * (2.0 ** sweep_octaves)).round(3) }

    attack = ParamOverride.fetch(o, :envelope, :attack) { rand(0.01..0.4).round(3) }
    decay = ParamOverride.fetch(o, :envelope, :decay) { rand(0.05..0.4).round(3) }
    sustain = ParamOverride.fetch(o, :envelope, :sustain) { rand(0.3..0.8).round(2) }
    release = ParamOverride.fetch(o, :envelope, :release) { rand(0.1..0.7).round(3) }
