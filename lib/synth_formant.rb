module SynthFormant
  ...
  # Vowel formant frequencies & bandwidths (Hz). Three formants per vowel.
  VOWELS = {
  ...
  def generate(duration: nil, freq: nil)
    ...
    fundamental = freq || rand(80.0..300.0).round(2)
    vowel = VOWELS.keys.sample
    formants = VOWELS[vowel]
    amplitude = rand(0.25..0.5).round(2)

    # Vibrato (subtle)
    vib_rate = rand(4.0..7.0).round(2)
    vib_depth = rand(0.0..0.015).round(4)

    # Pitch glide (small, vocal-like)
    glide_cents = rand(-100.0..100.0).round(2)
    end_freq = (fundamental * (2.0 ** (glide_cents / 1200.0))).round(3)

    # Per-formant relative amplitudes (formants 2 and 3 typically softer)
    f_amps = [1.0, rand(0.4..0.7).round(3), rand(0.2..0.5).round(3)]

    attack = rand(0.05..0.3).round(3)
    release = rand(0.15..0.5).round(3)
    ...
    params = {
      ...
      vowel: vowel,
      amplitude: amplitude,
      formants: formants.map.with_index { |(f, bw), i| { freq: f, bandwidth: bw, amplitude: f_amps[i] } },
      vibrato: { rate: vib_rate, depth: vib_depth },
      envelope: { attack: attack, release: release }
    }

    fof_lines = formants.each_with_index.map do |(f, bw), i|
      a = (amplitude * f_amps[i]).round(5)
      "  af#{i} fof2 #{a}, kfund, #{f}, 0, #{bw}, 0.003, 0.017, 0.007, 100, 1, 2, p3, 0, 0"
    end

    mix_expr = (0...formants.size).map { |i| "af#{i}" }.join(" + ")
